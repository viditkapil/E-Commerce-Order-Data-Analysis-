...
exec 2>&1 > /home/hadoop/Desktop/project_refine_data/logs/create_hive_dataset.log
...
hive -e " CREATE EXTERNAL TABLE if not exists $hive_database.$user_table 
...
hive -e " CREATE EXTERNAL TABLE if not exists $hive_database.$product_table
(url string, category string) 
ROW FORMAT DELIMITED FIELDS TERMINATED BY '\t' STORED AS TEXTFILE 
LOCATION '/user/hadoop/data/products'; "
status=$?
else 
...
hive -e " CREATE EXTERNAL TABLE if not exists $hive_database.$logs_table
(
...
ROW FORMAT DELIMITED FIELDS TERMINATED BY '\t' STORED AS TEXTFILE 
LOCATION '/user/hadoop/data/omniturelogs' ;" 
else 
echo "log table not created"
fi
